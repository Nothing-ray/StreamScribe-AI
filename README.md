...
本项目使用 **DeepSeek V4 Pro** 模型进行文本处理，该模型具有以下特点：

- **大上下文窗口** - 支持处理更长的文本块，减少分段数量
- **思考模式** - 可启用思考模式以获得更高质量的输出（默认启用）
- **强推理能力** - 相比旧版模型，在理解和处理复杂文本方面表现更优

**思考模式配置：**

思考模式可通过修改 `src/api_utils.py` 中的配置来控制：

```python
# 思考模式配置
ENABLE_THINKING_MODE = True   # 思考模式开关（设为 False 可禁用）
REASONING_EFFORT = "high"     # 思考强度: "high"（普通请求默认）或 "max"（复杂请求）
```

- **启用思考模式**：模型会先进行思考再输出最终答案，提升输出质量
- **禁用思考模式**：直接输出结果，响应速度更快
...
| 参数 | 说明 | 默认值 |
|------|------|--------|
| `min_spaces` | 每段最少空格数 | 150 |
| `max_spaces` | 每段最多空格数 | 180 |

**调整建议：**
- 输入文本较短 → 减小参数（如 100-120）
- 输入文本较长 → 增大参数（如 200-250）
- API 处理不稳定 → 减小参数
- 利用大上下文 → 可适当增大参数
...
4. API key 是否有效（前往 [DeepSeek 控制台](https://platform.deepseek.com/) 检查）
